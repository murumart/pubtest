...
func exit() -> void:
	if _loaded_from_file:
		...
		write_dict_to_global_file(loaded_dict, _file_path)
	done.emit()
	queue_free.call_deferred()


static func write_dict_to_global_file(data: Dictionary, filename: String) -> void:
...
func load_file(filename: String) -> void:
	var data := get_dict_from_global_file(filename)
	if not data:
		return
	load_dict(data)
...
static func get_dict_from_global_file(filename: String) -> Dictionary:
...
func _save_pressed() -> void:
	...
	filemenu.file_selected.connect(func(path: String) -> void:
		write_dict_to_global_file(loaded_dict, path)
	)
