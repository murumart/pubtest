...
func play_song(song: String, fade_speed := 1.0, options := {}):
	...
	if not song.is_empty():
		assert(song in list, "song with name %s doesn't exist" % song)
	var song_dict: Dictionary = list.get(song, {})
