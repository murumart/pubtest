...
func init(options := {}) -> void:
	var label := $Label
	var size: float = options.get("size", 3.0) # font size in multiples of 8
	...
	var scrsz := get_window().get_size_with_decorations()
	clamp_zone_max = Vector2(scrsz.x - 10, scrsz.y - 10)
